  ...
  Weber = (function() {
    ...
    Weber.prototype.server = function() {
      ...
      for (urlpath in _ref) {
        ...
        this.app.get(urlpath, js.createPackage(options, logger).createServer());
      }
      _ref2 = this.options.css;
      for (urlpath in _ref2) {
        options = _ref2[urlpath];
        this.app.get(urlpath, css.createPackage(options, logger).createServer());
      }
    ...
    Weber.prototype.init = function() {
      if (path.existsSync(this.options.conf)) {
        return logger.info("" + this.options.conf + " already exists");
      } else {
        fs.writeFileSync(this.options.conf, sample_conf);
        return logger.info("Wrote " + this.options.conf);
      }
    };
